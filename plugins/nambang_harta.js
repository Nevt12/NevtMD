...
let handler = async (m, { conn, usedPrefix, text }) => {
  //      let timerand = `${Math.floor(Math.random() * 259200000)}`.trim()
	    let time = global.db.data.users[m.sender].nambang + 28800000
  if (new Date - global.db.data.users[m.sender].nambang< 28800000) throw `Anda sudah menambang\nMohon tunggu hasil pertambangan mu\nTunggu selama ${msToTime(time - new Date())} lagi`
//    if (global.db.data.users[m.sender].aqua > 9) {
  ...
  //  global.db.data.users[m.sender].aqua -= aquah * 1
    ...
    global.db.data.users[m.sender].emasbiasa += emasbiasas * 1
    global.db.data.users[m.sender].emasbatang += emasbatangs * 1
